...
void initialize_console_peripheral(void)
{
    ...
    /* Install UART driver for interrupt-driven reads and writes. The RX buffer holds a few
     * whole XMODEM-1K blocks (1029 bytes each): `fs put` reads them raw from this driver, at
     * up to 2 Mbaud, and the example's 256 bytes would overflow within one block. */
